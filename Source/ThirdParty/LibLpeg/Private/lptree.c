...
static void fixonecall (lua_State *L, int postable, TTree *g, TTree *t) {
  ...
  assert(sib2(t)->tag == TLibLpegRule);
...
static void finalfix (lua_State *L, int postable, TTree *g, TTree *t) {
 tailcall:
  switch (t->tag) {
    case TLibLpegGrammar:  /* subgrammars were already fixed */
      return;
    case TLibLpegOpenCall: {
    ...
    case TLibLpegSeq: case TLibLpegChoice:
...
static void correctkeys (TTree *tree, int n) {
 ...
 tailcall:
  switch (tree->tag) {
    case TLibLpegOpenCall: case TLibLpegCall: case TLibLpegRunTime: case TLibLpegRule: {
      if (tree->key > 0)
        tree->key += n;
      break;
    }
...
static LpegPattern *getpattern (lua_State *L, int idx) {
  return (LpegPattern *)luaL_checkudata(L, idx, PATTERN_T);
}


static int getsize (lua_State *L, int idx) {
  return (lua_rawlen(L, idx) - sizeof(LpegPattern)) / sizeof(TTree) + 1;
}


static TTree *gettree (lua_State *L, int idx, int *len) {
  LpegPattern *p = getpattern(L, idx);
  if (len)
    *len = getsize(L, idx);
  return p->tree;
}
...
static TTree *newtree (lua_State *L, int len) {
  size_t size = (len - 1) * sizeof(TTree) + sizeof(LpegPattern);
  LpegPattern *p = (LpegPattern *)lua_newuserdata(L, size);
  luaL_getmetatable(L, PATTERN_T);
  lua_pushvalue(L, -1);
...
static TTree *seqaux (TTree *tree, TTree *sib, int sibsize) {
  tree->tag = TLibLpegSeq; tree->u.ps = sibsize + 1;
...
static void fillseq (TTree *tree, int tag, int n, const char *s) {
  ...
  for (i = 0; i < n - 1; i++) {  /* initial n-1 copies of Seq tag; Seq ... */
    tree->tag = TLibLpegSeq; tree->u.ps = 2;
    sib1(tree)->tag = tag;
...
static TTree *numtree (lua_State *L, int n) {
  if (n == 0)
    ...
    if (n > 0)
      ...
      n = -n;
      tree = newtree(L, 2 * n);
      tree->tag = TLibLpegNot;
      nd = sib1(tree);
    }
    fillseq(nd, TLibLpegAny, n, NULL);  /* sequence of 'n' any's */
    return tree;
  }
}
...
static TTree *getpatt (lua_State *L, int idx, int *len) {
  ...
  switch (lua_type(L, idx)) {
    case LUA_TSTRING: {
      ...
      if (slen == 0)  /* empty? */
        tree = newleaf(L, TLibLpegTrue);  /* always match */
      else {
        tree = newtree(L, 2 * (slen - 1) + 1);
        fillseq(tree, TLibLpegChar, slen, s);  /* sequence of 'slen' chars */
      }
      break;
    }
    case LUA_TNUMBER: {
    ...
    case LUA_TBOOLEAN: {
      tree = (lua_toboolean(L, idx) ? newleaf(L, TLibLpegTrue) : newleaf(L, TLibLpegFalse));
      break;
    }
    case LUA_TTABLE: {
    ...
    case LUA_TFUNCTION: {
      tree = newtree(L, 2);
      tree->tag = TLibLpegRunTime;
      tree->key = addtonewktable(L, 0, idx);
      sib1(tree)->tag = TLibLpegTrue;
      break;
    }
    default: {
...
static int lp_seq (lua_State *L) {
  ...
  if (tree1->tag == TLibLpegFalse || tree2->tag == TLibLpegTrue)
    lua_pushvalue(L, 1);  /* false . x == false, x . true = x */
  else if (tree1->tag == TLibLpegTrue)
    lua_pushvalue(L, 2);  /* true . x = x */
  else
    newroot2sib(L, TLibLpegSeq);
  return 1;
}
...
static int lp_choice (lua_State *L) {
  ...
  else if (nofail(t1) || t2->tag == TLibLpegFalse)
    lua_pushvalue(L, 1);  /* true / x => true, x / false => x */
  else if (t1->tag == TLibLpegFalse)
    lua_pushvalue(L, 2);  /* false / x => x */
  else
    newroot2sib(L, TLibLpegChoice);
  return 1;
}
...
static int lp_star (lua_State *L) {
  ...
  if (n >= 0) {  /* seq tree1 (seq tree1 ... (seq tree1 (rep tree1))) */
    ...
    for (; n > 1; n--) {  /* repeat (n - 1) times */
      tree->tag = TLibLpegChoice; tree->u.ps = n * (size1 + 3) - 2;
      sib2(tree)->tag = TLibLpegTrue;
      tree = sib1(tree);
      tree = seqaux(tree, tree1, size1);
    }
    tree->tag = TLibLpegChoice; tree->u.ps = size1 + 1;
    sib2(tree)->tag = TLibLpegTrue;
    memcpy(sib1(tree), tree1, size1 * sizeof(TTree));
  }
  copyktable(L, 1);
...
static int lp_sub (lua_State *L) {
  ...
  if (tocharset(t1, &st1) && tocharset(t2, &st2)) {
    ...
    tree->tag = TLibLpegSeq;  /* sequence of... */
    tree->u.ps =  2 + s2;
    sib1(tree)->tag = TLibLpegNot;  /* ...not... */
...
static int lp_behind (lua_State *L) {
  ...
  tree = newroot1sib(L, TLibLpegBehind);
  tree->u.n = n;
  return 1;
}
...
static int lp_V (lua_State *L) {
  TTree *tree = newleaf(L, TLibLpegOpenCall);
...
static int capture_aux (lua_State *L, int cap, int labelidx) {
  TTree *tree = newroot1sib(L, TLibLpegCapture);
  tree->cap = cap;
...
static int lp_constcapture (lua_State *L) {
  ...
  if (n == 0)  /* no values? */
    newleaf(L, TLibLpegTrue);  /* no capture */
  else if (n == 1)
...
static int lp_matchtime (lua_State *L) {
  TTree *tree;
  luaL_checktype(L, 2, LUA_TFUNCTION);
  tree = newroot1sib(L, TLibLpegRunTime);
...
static void buildgrammar (lua_State *L, TTree *grammar, int frule, int n) {
  ...
  nd->tag = TLibLpegTrue;  /* finish list of rules */
}
...
static int checkloops (TTree *tree) {
 tailcall:
  if (tree->tag == TLibLpegRep && nullable(sib1(tree)))
    return 1;
  else if (tree->tag == TLibLpegGrammar)
...
static int verifyrule (lua_State *L, TTree *tree, int *passed, int npassed,
 ...
 tailcall:
  switch (tree->tag) {
    ...
    case TLibLpegTrue:
    case TLibLpegBehind:  /* look-behind cannot have calls */
      return 1;
    case TLibLpegNot: case TLibLpegAnd: case TLibLpegRep:
    ...
    case TLibLpegCall:
      ...
      tree = sib2(tree); goto tailcall;
    case TLibLpegSeq:  /* only check 2nd child if first is nb */
      if (!verifyrule(L, sib1(tree), passed, npassed, 0))
        return nb;
      /* else return verifyrule(L, sib2(tree), passed, npassed, nb); */
      tree = sib2(tree); goto tailcall;
    case TLibLpegChoice:  /* must check both children */
...
static Instruction *prepcompile (lua_State *L, LpegPattern *p, int idx) {
...
static int lp_match (lua_State *L) {
  ...
  const char *r;
  size_t l;
  LpegPattern *p = (getpatt(L, 1, NULL), getpattern(L, 1));
