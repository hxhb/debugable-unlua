...
int tocharset (TTree *tree, Charset *cs) {
  switch (tree->tag) {
    case TLibLpegSet: {  /* copy set */
      loopset(i, cs->cs[i] = treebuffer(tree)[i]);
      return 1;
    }
    case TLibLpegChar: {  /* only one char */
...
int hascaptures (TTree *tree) {
 tailcall:
  switch (tree->tag) {
    case TLibLpegCapture: case TLibLpegRunTime:
      return 1;
    case TLibLpegCall:
      return callrecursive(tree, hascaptures, 0);
    case TLibLpegRule:  /* do not follow siblings */
      tree = sib1(tree); goto tailcall;
    case TLibLpegOpenCall: assert(0);
    default: {
...
int checkaux (TTree *tree, int pred) {
 tailcall:
  switch (tree->tag) {
    case TLibLpegChar: case TLibLpegSet: case TLibLpegAny:
    case TLibLpegFalse: case TLibLpegOpenCall:
      return 0;  /* not nullable */
    case TLibLpegRep: case TLibLpegTrue:
      return 1;  /* no fail */
    case TLibLpegNot: case TLibLpegBehind:  /* can match empty, but can fail */
      if (pred == PEnofail) return 0;
      else return 1;  /* PEnullable */
    case TLibLpegAnd:  /* can match empty; fail iff body does */
      if (pred == PEnullable) return 1;
      /* else return checkaux(sib1(tree), pred); */
      tree = sib1(tree); goto tailcall;
    case TLibLpegRunTime:  /* can fail; match empty iff body does */
      if (pred == PEnofail) return 0;
      /* else return checkaux(sib1(tree), pred); */
      tree = sib1(tree); goto tailcall;
    case TLibLpegSeq:
    ...
    case TLibLpegChoice:
      ...
      /* else return checkaux(sib1(tree), pred); */
      tree = sib1(tree); goto tailcall;
    case TLibLpegCapture: case TLibLpegGrammar: case TLibLpegRule:
      /* return checkaux(sib1(tree), pred); */
      tree = sib1(tree); goto tailcall;
    case TLibLpegCall:  /* return checkaux(sib2(tree), pred); */
      tree = sib2(tree); goto tailcall;
...
int fixedlen (TTree *tree) {
 ...
 tailcall:
  switch (tree->tag) {
    case TLibLpegChar: case TLibLpegSet: case TLibLpegAny:
      return len + 1;
    case TLibLpegFalse: case TLibLpegTrue: case TLibLpegNot: case TLibLpegAnd: case TLibLpegBehind:
      return len;
    case TLibLpegRep: case TLibLpegRunTime: case TLibLpegOpenCall:
      return -1;
    case TLibLpegCapture: case TLibLpegRule: case TLibLpegGrammar:
...
static int getfirst (TTree *tree, const Charset *follow, Charset *firstset) {
 tailcall:
  switch (tree->tag) {
    ...
    case TLibLpegRunTime: {  /* function invalidates any follow info. */
    ...
    case TLibLpegBehind: {  /* instruction gives no new information */
...
static int headfail (TTree *tree) {
 tailcall:
  switch (tree->tag) {
    case TLibLpegChar: case TLibLpegSet: case TLibLpegAny: case TLibLpegFalse:
      return 1;
    case TLibLpegTrue: case TLibLpegRep: case TLibLpegRunTime: case TLibLpegNot:
    case TLibLpegBehind:
      return 0;
    case TLibLpegCapture: case TLibLpegGrammar: case TLibLpegRule: case TLibLpegAnd:
...
static int needfollow (TTree *tree) {
 tailcall:
  switch (tree->tag) {
    case TLibLpegChar: case TLibLpegSet: case TLibLpegAny:
    case TLibLpegFalse: case TLibLpegTrue: case TLibLpegAnd: case TLibLpegNot:
    case TLibLpegRunTime: case TLibLpegGrammar: case TLibLpegCall: case TLibLpegBehind:
      return 0;
    case TLibLpegChoice: case TLibLpegRep:
      return 1;
    case TLibLpegCapture:
      tree = sib1(tree); goto tailcall;
    case TLibLpegSeq:
      tree = sib2(tree); goto tailcall;
    default: assert(0); return 0;
...
typedef struct CompileState {
  LpegPattern *p;  /* pattern being compiled */
...
void realloccode (lua_State *L, LpegPattern *p, int nsize) {
  void *ud;
...
static void codechoice (CompileState *compst, TTree *p1, TTree *p2, int opt,
                        const Charset *fl) {
  int emptyp2 = (p2->tag == TLibLpegTrue);
  Charset cs1, cs2;
...
static void codegrammar (CompileState *compst, TTree *grammar) {
  ...
  for (rule = sib1(grammar); rule->tag == TLibLpegRule; rule = sib2(rule)) {
    positions[rulenumber++] = gethere(compst);  /* save rule position */
    codegen(compst, sib1(rule), 0, NOINST, fullset);  /* code rule */
    addinstruction(compst, IRet, 0);
  }
  assert(rule->tag == TLibLpegTrue);
...
static void codegen (CompileState *compst, TTree *tree, int opt, int tt,
 ...
 tailcall:
  switch (tree->tag) {
    case TLibLpegChar: codechar(compst, tree->u.n, tt); break;
    case TLibLpegAny: addinstruction(compst, IAny, 0); break;
    case TLibLpegSet: codecharset(compst, treebuffer(tree), tt); break;
    case TLibLpegTrue: break;
    case TLibLpegFalse: addinstruction(compst, IFail, 0); break;
    case TLibLpegChoice: codechoice(compst, sib1(tree), sib2(tree), opt, fl); break;
    case TLibLpegRep: coderep(compst, sib1(tree), opt, fl); break;
    case TLibLpegBehind: codebehind(compst, tree); break;
    case TLibLpegNot: codenot(compst, sib1(tree)); break;
    case TLibLpegAnd: codeand(compst, sib1(tree), tt); break;
    case TLibLpegCapture: codecapture(compst, tree, tt, fl); break;
    case TLibLpegRunTime: coderuntime(compst, tree, tt); break;
    case TLibLpegGrammar: codegrammar(compst, tree); break;
    case TLibLpegCall: codecall(compst, tree); break;
    case TLibLpegSeq: {
...
** Compile a pattern
*/
Instruction *compile (lua_State *L, LpegPattern *p) {
  CompileState compst;
